...
export default function Home() {
  return <main>
    ...
    <nav className="dock" aria-label="Story chapters">
    ...
    <section className="chapter garden" id="archive">
      ...
      <p className="cat-note">The archivist is pretending not to watch you.</p>
    </section>
    ...
    <section className="chapter mission" id="mission">
      ...
      <div className="logs">{[["Origin","Two people encountered online."],["Complication","Distance. Deeply inconvenient."],["Recovery protocol","Honesty, reassurance, patience, trying again."],["Partner status","Still my favorite person."],["Destination","Together."]].map(([a,b])=><div key={a}><span>{a}</span><p>{b}</p></div>)}</div></div>
    </section>
    ...
    <section className="letter" id="letter"><div className="candle" aria-hidden="true"/><p className="chapter-no">A letter sent across time</p><h2>Dear Lia,</h2>
      ...
      <button className="audio" type="button" disabled><span>▶</span><b>Birthday voice note</b><small>Audio pending review · never autoplays</small></button>
    </section>

    <section className="finale" id="finale"><div className="dawn"/><p className="kicker">Primary celebration signal · 07.22</p><h2>Happy Birthday,<br/><em>Lia.</em></h2><p>In every timeline, I would still look for you.</p><div className="ferrets" aria-label="Placeholder for two dancing ferret mascots"><span>〰</span><b>♡</b><span>〰</span></div><p className="final-line">The day this universe became softer because you entered it.</p><a href="#signal">Return to the signal ↑</a></section>
  </main>;
}
